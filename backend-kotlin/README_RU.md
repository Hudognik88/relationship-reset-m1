...
HTTPS и пропускает старый API, `/client/*` и три явных ресурса `/rehearsal/`.
Клиентская репетиция описана в [CLIENT_REHEARSAL_RU.md](CLIENT_REHEARSAL_RU.md):
одноразовое приглашение, собственная анкета, публикация разбора человеком.
Она использует отдельные таблицы и только вымышленные данные.
...
HTTPS, авторизация, сохранность тестовой анкеты и изоляция двух клиентских
сеансов проверяются после запуска.
...
восстанавливает его в новую `rr_restore_test_*` и сравнивает все значения
известных таблиц (трёх исходных и четырёх клиентских после миграции).
Рабочая staging-база не изменяется. Копия и проверочная база остаются
